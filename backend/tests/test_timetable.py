...
def test_timetable_multi_segment_cumulative(
    ...
    assert stops[0]["departure_time"] == "2026-05-01T10:00"
    assert stops[1]["arrival_time"] == "2026-05-01T11:00"
    assert stops[2]["arrival_time"] == "2026-05-01T13:00"
    assert stops[3]["arrival_time"] == "2026-05-01T16:00"
...
def test_timetable_stop_duration_added(
    ...
    # For segment 0 (AB): travel 1h -> arrive B at 11:00, then stop_duration=10min -> depart B at 11:10
    # For segment 1 (BC): travel 2h -> arrive C at 13:10, then stop_duration=30min -> depart C at 13:40
    assert stops[1]["departure_time"] == "2026-05-01T11:10"
    assert stops[2]["arrival_time"] == "2026-05-01T13:10"
    assert stops[2]["departure_time"] == "2026-05-01T13:40"
...
def test_timetable_midnight_crossing(db: None) -> None:
    ...
    dep = Departure.objects.create(
        train=train,
        date=date.fromisoformat("2026-05-01"),
        departure_time=time.fromisoformat("23:30"),
    )
