...
def test_resolve_station_range_valid(
    route: Route, stations: list[Station],
) -> None:
...
def _book(
    departure: Departure, seat: Seat,
    station_from: Station, station_to: Station,
) -> None:
    """Create a minimal Booking for testing (bypasses service layer)."""
    order = Order.objects.create()
    passenger = Passenger.objects.create(
        name="Test", passport_number="T", gender="male", birth_date=date(1990, 1, 1),
    )
    Booking.objects.create(
        order=order, departure=departure, seat=seat,
        station_from=station_from, station_to=station_to, passenger=passenger,
    )


@pytest.mark.django_db
def test_free_seat_ids_all_free(
    departure: Departure, seat: Seat, seat2: Seat,
) -> None:
...
def test_free_seat_ids_adjacent_non_overlapping(
    stations: list[Station], departure: Departure, seat: Seat, seat2: Seat,
) -> None:
